...
class Writer():
    ...
    def __init__(self, device: framebuf.FrameBuffer, font,
                 invert_mono=False,
                 wrap=True, row_clip=None, col_clip=None,
                 swap_dimensions=False,
                 verbose=True):
        self.devid = _get_id(device)
        self.device: framebuf.FrameBuffer = device
        ...
        if verbose:
            print(f"Font Orientation: Horizontal. Reversal: {font.reverse()}.")
            print(f"Device Dimensions: {device.width}x{device.height}. Swapped: {swap_dimensions}.")
            print(f'Start row = {self._getstate().text_row} col = {self._getstate().text_col}')
        self.screenwidth = device.width if not swap_dimensions else device.height  # In pixels
        self.screenheight = device.height if not swap_dimensions else device.width
        self.bgcolor = 0 if not invert_mono else 1  # Monochrome background and foreground colors
        self.fgcolor = 1 if not invert_mono else 0
        self.invert_mono = invert_mono
        self.row_clip = row_clip  # Clip or scroll when screen full
        self.col_clip = col_clip  # Clip or new line when row is full
        self.wrap = wrap  # Word wrap
        self.cpos = 0
        self.tab = 4
    ...
    def printstring(self, string, invert: bool | None = None):
    ...
    def _get_char(self, char, recurse):
        ...
        if oh > 0:
            if self.col_clip or self.wrap:
                np = char_width - oh - 1  # No. of printable columns
                if np <= 0:
                    return
            else:
                self._newline()
    ...
    def _printchar(self, char, invert=None, recurse=False):
        s = self._getstate()
        self._get_char(char, recurse)
        if self.glyph is None:
            return  # All done
        buf = bytearray(self.glyph)
        if invert or (self.invert_mono and (invert is not False)):
